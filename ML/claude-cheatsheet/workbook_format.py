...
.sol-body{padding:1rem 1.15rem 1.2rem}
.sol-body h4{color:var(--purple);font-size:.9rem;margin:.65rem 0 .3rem}
.sol-body .step,.sol-body .ans,.sol-body .formula,.sol-body .diagram{margin-top:.45rem}
...
<p>Also: (i) For weather prediction, identify Mitchell's <em>E, T, P</em>. Is predicting rainfall amount regression or classification? (ii) AlphaGo — was the first version supervised or reinforcement?</p></div>""",
    "type2": """<div class="q-prompt"><p><strong>Question.</strong> Inspect each messy dataset table. List at least 5–6 <em>data quality issues</em> (missing values, outliers, duplicates, inconsistent formats, invalid types, etc.) and state how you would fix each <em>without writing code</em>.</p></div>""",
...
<ol><li>(a)(i) Interpret each coefficient in dollars. (ii) If \(X_1\) is re-recorded in Wh instead of kWh, what happens to \(w_1\)?</li>
<li>(b) Given \(\\partial J/\\partial\\theta_3 = -12\), which direction should GD move \(\\theta_3\)? Write the update.</li>
<li>(c) Which feature appears most influential? What caveat applies before comparing raw magnitudes?</li></ol></div>""",
    2: """<div class="q-prompt"><p><strong>Question.</strong> Loan approval dataset: 10 applicants — 6 Approved, 4 Denied.</p>
<ol><li>Compute parent entropy \(H(S)\) using log base 2.</li>
<li>(Bonus) Split on <em>Existing Loan = No</em>: 5 Approved / 0 Denied vs 1 Approved / 4 Denied. Compute IG.</li></ol></div>""",
    3: """<div class="q-prompt"><p><strong>Question.</strong> Credit scores in training set: min=590, max=710 (range 120). Samples S1=620, S2=680, S3=590, S4=710.</p>
<ol><li>Min-max normalize each sample to [0,1].</li>
<li>Normalize a new applicant with score 650.</li>
<li>Write the logistic regression gradient update for weights (cross-entropy loss).</li></ol></div>""",
    4: """<div class="q-prompt"><p><strong>Question.</strong> Fraud detection on 10,000 transactions (100 fraud). Model A: TP=45, FN=55, FP=90, TN=9810.</p>
<p>Compute accuracy, precision, recall, FPR, and F1. Why can accuracy mislead here? Which model trait matters for fraud?</p></div>""",
    5: """<div class="q-prompt"><p><strong>Question.</strong> Spam filter comparison:</p>
<table><tr><th>Model</th><th>Train acc</th><th>Test acc</th></tr>
<tr><td>A (2 features)</td><td>61%</td><td>60%</td></tr>
<tr><td>B (47 features)</td><td>99%</td><td>67%</td></tr></table>
<p>Diagnose bias vs variance for each model and suggest fixes.</p></div>""",
}
...
def wrap_q_block(inner: str) -> str:
    ...
    if sol:
        # Keep sub-part headings visible in the question stem
...
def inject_prompt(stem: str, prompt: str) -> str:
    if not prompt or "q-prompt" in stem:
        return stem
